...
本套件只有一张素材(散兵 × 荧), 提供三种摆法:

  genshen-cp20                 # 默认: single1 卡片式(模糊背景 + 居中圆角卡片)
  genshen-cp20 2 / 3           # 第 2 / 第 3 张(按张编号)
  genshen-cp20 cover           # 第 1 张满屏: cover 裁切铺满整屏, 无边框
  genshen-cp20 showall1        # 第 1 张完整: 等比放进纯色底, 一个像素都不裁
  genshen-cp20 card            # 第 1 张卡片式(简写, 等价 single1)
  genshen-cp20 list            # 列出所有可切换样式
  genshen-cp20 random          # 随机来一张
  genshen-cp20 all             # 生成全部样式到 ~/.genshen-cp20/wallpapers
  genshen-cp20 all --out DIR   # 生成到指定目录(JetBrains 背景图用)
  genshen-cp20 cycle 30        # 每 30 分钟自动随机换壁纸
  genshen-cp20 switcher        # 打开可视化切换器
  genshen-cp20 pet             # 启动桌面桌宠
  genshen-cp20 deepking        # 生成 DeepKing 界面皮肤 + 离线预览
  genshen-cp20 copy            # 只合成不设置
...
def build_parser():
    ...
    # 每一种模式都注册成子命令。
    # argparse 的子命令是"贪婪"的: 一旦注册了子解析器, `genshen-cp20 cover`
    ...
    # 覆盖掉子解析器 set_defaults 设的 mode, 于是 `genshen-cp20 cover2` 反而
