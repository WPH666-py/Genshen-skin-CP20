...
def mode_label(mode):
    ...
    归一成 `showall1`, 直接查表就落散兵、界面会显示生涩的 "showall1"。
...
def compose_showall(idx, size=None):
    ...
    底色取素材四角的中位色, 因此白底插画得到白边、夜散兵插画得到深边。
    """
    ensure_pillow()
    from PIL import Image
...
def build_all(out_dir=None, size=None, force=True):
    """生成全部模式到 out_dir(默认 ~/.genshen-cp20/wallpapers), 返回 [(mode, label, path)]。"""
    ensure_dirs()
    size = tuple(size) if size else screen_size()
