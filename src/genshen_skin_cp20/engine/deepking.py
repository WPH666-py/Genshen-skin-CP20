...
def pkg_root():
    """包目录 .../genshen_skin_cp20(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp20.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #eeedf3;
  --bg-layer-3: #dcdbe8;
  --label-primary: #1e1c31;
  --label-secondary: #474567;
  --label-tertiary: #787698;
  --brand-primary: #3f3c63;
  --accent: #3f3c63;
  --primary: #3f3c63;
  --interactive-bg-hover-solid: #e4e3ef;
  --interactive-bg-active: #cfcee4;
  --border-l1: #e1e0ec;
  --border-l2: #c8c6dd;
  --border: #c8c6dd;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP20</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP20", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP20", mascot.get("dark")),
    }
