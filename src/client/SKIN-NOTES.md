...
本目录的 `genshen-cp20.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp20.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp20 deepking          # 导出 ~/.genshen-cp20/deepking/*.skin.json
  ```
...
```bash
genshen-cp20 deepking --check    # 应输出「通过」
genshen-cp20 deepking --what     # 看转换器实际会取到什么值
```
...
- `mascot-cp20-light.jpg` —— 取左侧双人(荧+荧), 配浅色界面
- `mascot-cp20-dark.jpg` —— 取右侧双人(荧+绫华), 配深色界面
