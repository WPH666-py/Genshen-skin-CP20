...
单张素材 x 三种摆法(共 9 种壁纸) / 桌面桌宠 / 多 IDE 皮肤 / DeepKing 界面皮肤
    ...
    import genshen_skin_cp20 as gs
    gs.build("single1")          # 卡片式(默认), 返回文件路径
    gs.build("cover1")           # 满屏
    gs.build("showall")          # 完整不裁
    gs.set_wallpaper(path)       # 设为系统壁纸

本包与 CP1/CP2/CP3 并列, 命名散兵间完全隔离:
包名 genshen-skin-cp20 / 命令 genshen-cp20 / 运行时目录 ~/.genshen-cp20,
两个套件可同时安装、各自切换。
"""
from .characters.cp20_pair import (  # noqa: F401
    APP_DIR,
    APP_NAME,
    APP_SLUG,
    DEFAULT_MODE,
