...
try:
    from genshen_skin_cp20.engine import skin_core as sc
...
def make_thumbs():
    ...
    # 拼版总览图: 按实际样式数量排成一行(本套件 3 个), 不留散兵行
    modes_n = len(modes)
...
def make_icon():
    ...
    # 渐变底: 上深靛紫 -> 下深靛墨(与皮肤 accent #3f3c63 同源)
    top = (63, 60, 99)     # #3f3c63 散兵的深靛紫
    bottom = (30, 28, 49)   # #1e1c31 深靛墨
    for y in range(S):
        t = y / (S - 1)
